...
def admin_dashboard():
    try:
        ...
        high_priority = complaints_col.count_documents({"priority": {"$in": ["High", "Critical"]}})
        resolved = complaints_col.count_documents({"status": "Resolved"})

        departments = [
            {"department": row["_id"] or "Unassigned", "count": row["count"]}
        ...
        for row in rows:
            complaints.append({
                ...
                "created_at": row["created_at"].strftime("%d %b %Y, %I:%M %p") if row.get("created_at") else None,
...
def campus_signals():
    try:
        if DISABLE_DB:
            return jsonify({"success": True, "signals": [], "mode": "mock"})

        complaints_col = get_complaints()
        priority_map = {4: "Critical", 3: "High", 2: "Medium", 1: "Low", 0: "Unknown"}

        rows = complaints_col.aggregate([
            {
                "$match": {
                    "location": {"$nin": [None, "Unknown"]}
                }
            },
            {
                "$group": {
            ...
            {"$match": {"complaint_count": {"$gte": 2}}},
            {"$sort": {"complaint_count": -1, "average_severity": -1}},
            {"$limit": 10}
        ])
        ...
        for row in rows:
            signals.append({
                ...
                "average_severity": round(float(row["average_severity"]), 1) if row["average_severity"] is not None else 0,
                "priority": priority_map.get(row["priority_level"], "Unknown")
            })
